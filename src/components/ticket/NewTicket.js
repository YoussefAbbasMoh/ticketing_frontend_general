  ...
  const handleImageChange = (e) => {
    ...
    files.forEach(file => {
      if (!allowedTypes.includes(file.type)) {
        setError(`File ${file.name} is not a valid image type. Please use JPEG, PNG, GIF, or WebP.`);
        return;
      }
    ...
    try {
      ...
      if (images.length > 0) {
        ...
        try {
          ...
          setError(uploadError.response?.data?.message || 'Failed to upload images. Please try again.');
                ...
                <Card.Content className="p-6">
                  ...
                  <div className="space-y-5">
                    ...
                    <div className="p-4 rounded-lg bg-gradient-to-r from-secondary/5 to-secondary/10 border-2 border-secondary/20">
                      <p className="text-xs font-medium text-gray-600 mb-1">Ticket ID (Auto-generated)</p>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                      ...
                      <div className="w-full">
                        <label className="block text-sm font-medium text-gray-700 mb-1.5">
                          Assign To (Project Team)<span className="text-red-500 ml-1">*</span>
                        </label>
                        <div className="relative">
                          ...
                          >
                            <option value="">
                              {fetchingData ? 'Loading team members...' : users.length === 0 ? 'No team members assigned' : 'Select a team member...'}
                            </option>
                            {users.map((userItem) => (
                        ...
                        {users.length === 0 && !fetchingData && (
                          <p className="mt-1.5 text-xs text-amber-600">
                            ⚠️ No team members assigned to this project. Please contact an admin to assign users.
                          </p>
                        )}
                      </div>
                      ...
                      <Input
                        ...
                        disabled
                        required
                        placeholder="Auto-filled from assigned user"
                        icon={
                          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                          </svg>
                    ...
                    <div className="w-full">
                      ...
                      <div className="relative">
                        ...
                        >
                          ...
                          <option value="take_your_time_3-5_days">🟢 Take your time 3-5 days</option>
                        </select>
                      </div>
                    </div>
                        ...
                        <div className="relative">
                          ...
                          >
                            ...
                            <span className="text-sm text-gray-600 font-medium">
                              {uploadingImages ? 'Uploading images...' : 'Click to upload images (Max 5MB each, up to 10 images)'}
                            </span>
                          </label>
                        </div>
              ...
              <div className="sticky top-6 space-y-6">
                ...
                <Card>
                  <Card.Content className="p-5">
                    <p className="text-sm font-medium text-gray-600 mb-3">Ticket Status</p>
                    <div className="w-full">
                      <select
                        name="status"
                    ...
                    <div className="mt-3">
                      {formData.status === 'open' && <Badge variant="success" className="w-full justify-center">Open</Badge>}
                      {formData.status === 'in_progress' && <Badge variant="warning" className="w-full justify-center">In Progress</Badge>}
                      {formData.status === 'pending' && <Badge variant="orange" className="w-full justify-center">Pending</Badge>}
                      {formData.status === 'resolved' && <Badge variant="info" className="w-full justify-center">Resolved</Badge>}
                      {formData.status === 'closed' && <Badge variant="default" className="w-full justify-center">Closed</Badge>}
                    </div>
                  </Card.Content>
                </Card>
                ...
                <Card>
                  <Card.Content className="p-5">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <p className="text-sm font-medium text-gray-600">Support Handlers</p>
                    </div>
                    ...
                    <div className="w-full mb-3">
                      ...
                      >
                        <option value="" disabled>
                          {fetchingData ? 'Loading...' : users.length === 0 ? 'No team members' : 'Select a handler to add...'}
                        </option>
                        {users
                    ...
                    {formData.handler.length > 0 ? (
                        ...
                        <div className="p-3 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border-2 border-blue-200">
                          <div className="flex items-center justify-between mb-2">
                            <p className="text-xs font-semibold text-blue-900">
                              Selected Handlers ({formData.handler.length})
                            </p>
                          </div>
                      ...
                      <div className="p-4 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300 text-center">
                        ...
                        <p className="text-xs text-gray-500">No handlers selected</p>
                        <p className="text-xs text-gray-400 mt-1">Use dropdown above to add handlers</p>
                      </div>
                    )}
                  </Card.Content>
                </Card>
                ...
                <div className="p-5 rounded-xl bg-gradient-to-br from-secondary/5 to-secondary/10 border-2 border-secondary/20">
                  ...
                  <ul className="space-y-2">
                    <li className="text-xs text-gray-600 flex items-start gap-2">
                      ...
                      <span>Notification email sent to contact address</span>
                    </li>
                    <li className="text-xs text-gray-600 flex items-start gap-2">
                      <span className="text-secondary mt-0.5">•</span>
                      <span>Handlers will be CC'd on all emails</span>
                    </li>
                    <li className="text-xs text-gray-600 flex items-start gap-2">
                      <span className="text-secondary mt-0.5">•</span>
                      <span>Ticket ID is auto-generated</span>
                    </li>
                    <li className="text-xs text-gray-600 flex items-start gap-2">
                      <span className="text-secondary mt-0.5">•</span>
                      <span>Add multiple handlers via dropdown</span>
                    </li>
                  </ul>
                </div>
              </div>
              ...
              <Card>
                <Card.Content className="p-5">
                  <div className="flex gap-3 justify-end">
                    ...
                    >
                      {loading ? 'Creating...' : success ? 'Created!' : 'Create Ticket'}
                    </Button>
                  </div>
                </Card.Content>
              </Card>
